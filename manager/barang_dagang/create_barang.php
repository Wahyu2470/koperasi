            ...
            <div class="form-panel">
              ...
              <form class="form-horizontal style-form" method="post" enctype="multipart/form-data">
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select class="form-control custom-select input-lg" name="supplier">
                    <option disabled selected>-- Pilih Nama Supplier --</option>
                     <?php $prd=mysqli_query($koneksi,"SELECT * from supplier order by id_supplier desc");
                    while ($dataprd=mysqli_fetch_array($prd)){
                    ?>
                    <option value="<?php echo $dataprd['id_supplier']; ?>" title="<?php echo $dataprd['id_supplier'];?>"><?php echo $dataprd['nama_supplier']?></option>
                  <?php } ?>
                    </select>
                    </div>
                    </select>
              ...
              if (isset($_POST["simpan"])) {
                ...
                $koneksi->query("UPDATE barang_masuk set jumlah_kilo='$sisa' where id_barang='$nm_brg'") or die (mysqli_error($koneksi));
